package functional.v01;

public class Example {

   final static double[] EXPECTED_SALES_JAN_TO_DEC = { 34.0, 45.0, 67.0, 80.0, 34.0, 45.0, 67.0, 80.0, 34.0, 45.0, 67.0, 80.0 };

   // public static void main(String[] args) {
   //
   // final Sales sales = new Sales(EXPECTED_SALES_JAN_TO_DEC);
   // final FixedCosts fixedCosts = new FixedCosts(15.0);
   // final IncrementalCosts incrementalCosts = new IncrementalCost(5.1, 0.15);
   //
   // double total = 0.0;
   //
   // final Profit profit = new Profit(sales, fixedCosts, incrementalCosts);
   //
   // for (int i = 0; i < 12; i++) {
   // total += profit.valueAt(i);
   // }
   //
   // System.out.println("The total profit is: " + total);
   // }
}
